...
use vivace_resolver::version::{looks_too_strict, parse_name_version_pairs};
...
fn update_file(
    ...
    let mut text = vivace_core::phpjson::php_json_encode_with(
        &definition,
        vivace_core::phpjson::FLAGS_JSONFILE,
    )?;
    if indent != "    " {
...
fn platform_exception_details(
    ...
    use vivace_resolver::constraint::{Constraint, Op};
    let pkg = &session.arena[candidate];
    let mut details = Vec::new();
    for link in pkg.requires.iter() {
        if !vivace_resolver::platform::is_platform_package(&link.target) {
            continue;
        }
...
pub fn run_require(args: &RequireArgs) -> anyhow::Result<i32> {
    ...
    for (name, constraint) in &requirements {
        ...
        if let Err(e) = vivace_resolver::constraint::parse_constraints(constraint) {
            anyhow::bail!("{e}");
        }
    }
    ...
    {
        Some(vivace_resolver::pool::UpdateMode::ListedWithTransitiveDeps)
    } else if args.update_with_dependencies
        || args.with_dependencies
        || env_flag("COMPOSER_WITH_DEPENDENCIES")
    {
        Some(vivace_resolver::pool::UpdateMode::ListedWithTransitiveDepsNoRootRequire)
    } else {
        None
    };
...
fn update_requirements_after_resolution(
    ...
    for (package_name, lower) in to_guess.iter().zip(&lower_names) {
        ...
        let (pkg, _) = vivace_resolver::loader::load(entry, Origin::Locked, false)
    ...
    if let (Some(lock_text), true) = (lock_now, lock.is_some() && lock_enabled) {
        ...
        vivace_resolver::root::extract_stability_flags(
            &requirements,
...
fn locker_update_hash(
    ...
    let indent = vivace_resolver::json_manipulator::detect_indenting(lock_text)
        .map_err(|e| anyhow::anyhow!("{e}"))?;
    ...
    if let Some(Value::Object(m)) = data.get("stability-flags") {
        if !m.contains_key(vivace_core::phpjson::STDCLASS_MARKER) {
