...
| v0.8 `--dry-run` + lignes d'opérations + rapport post-update | fait localement (2026-09-15) : `steps.sh` compare stderr par défaut (202 cas, 180 avec stderr identique), `@dry-install` ; `RootPatch` pour `require`/`remove --dry-run` ; lock virtuel pour la phase d'installation d'un dry run ; dépôts `path` → v0.9 | docs/plans/v0.8-dry-run-path.md |
